...
- `"db:main"` → `myapp/service/db/main:latest`
...
| `compose_name` | Name as it appears in `docker-compose.yml` — colons replaced with dashes: `"db:main"` → `"db-main"`. Override with `compose.service_name`. |
...
| `services:db:main` | Service `"db:main"` |
