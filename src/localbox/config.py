...
class Solution:
    ...
    def service_image_tag(self, image_name: str) -> str:
        """Return the full image tag for a service image.

        Without registry: {solution}/service/{image_name}:latest
        With registry:    {registry}/{solution}/service/{image_name}:latest
        """
        base = f"{self.name}/service/{image_name}:latest"
        if self.registry:
