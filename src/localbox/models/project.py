...
def extract_repo_name(url: str) -> str:
    ...
    Examples:
        git@bitbucket.org:org/mylib.git -> mylib
...
class Project:
    ...
    def resolve_source_dir(self, projects_dir: Path) -> Path:
        ...
        (e.g. ``myservice`` from ``git@github.com:org/myservice.git``),
...
class JavaArtifact:
    ...
    Usage:
        webapps={
            ...
            "api": backend.api.artifact(),

            # Explicit path (relative to project root)
            "auth": backend.auth.artifact("auth-server/target/auth.war"),
        }
    """
