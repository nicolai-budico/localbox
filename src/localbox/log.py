...
File logging is always active once a solution root is known.
Logs are written to <solution_root>/.logs/localbox.log with size-based rotation.
Pass verbose=True to also stream logs to stderr (enabled by --verbose).
"""
...
def setup_logging(solution_root: Path, *, verbose: bool = False) -> None:
    """Attach a rotating file handler for the given solution root.

    When verbose=True, also streams DEBUG+ to stderr.
    Safe to call multiple times — only initialises once per process.
    """
    global _file_handler_id, _stderr_handler_id
