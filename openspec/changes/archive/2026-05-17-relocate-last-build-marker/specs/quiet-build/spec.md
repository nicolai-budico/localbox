...
#### Scenario: Last-build marker stored outside project repo
- **WHEN** a project build succeeds
- **THEN** a timestamp file SHALL be written to `.build/last-build/<project-name>` (inside the solution build directory, not the project source directory)

#### Scenario: Legacy marker cleaned up on successful build
- **WHEN** a project build succeeds and a `.last-build` file exists inside the project source directory
- **THEN** that file SHALL be deleted
