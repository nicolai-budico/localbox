...
Defines the behaviour of `localbox override list`, which displays every overridable identifier in the current solution together with its default value and any active override value. The command helps developers discover what can be configured and immediately spot required fields that have not yet been set.
