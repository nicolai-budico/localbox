...
Defines the behaviour of `localbox override clear <identifier>`, which reverts a single overridden field in `solution-override.py` back to its default by commenting out the active line (or restoring the `# REQUIRED` annotation for required fields), while leaving the rest of the file untouched.
