# gradle-builder-tasks Specification

## Purpose

Defines a declarative way to append extra Gradle tasks or CLI arguments to the default Gradle build command without replacing it wholesale. Covers `GradleBuilder`, `GradleWrapperBuilder`, and their `gradle()` / `gradlew()` factory functions, and specifies the rejection behavior on Maven builders.
